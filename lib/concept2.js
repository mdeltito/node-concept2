 ...
 *
 * @param {object} [options] - configuration options for the HID connection.
 * @param {string} [options.driver=libusb] - underlying HID driver (linux only)
 * @param {int} [options.index=0] - the device index
 */
class Concept2 extends EventEmitter {
  constructor({ driver = "libusb", index = 0 } = {}) {
    super();
    this.VENDOR_ID = 6052;
    HID.setDriverType(driver);
